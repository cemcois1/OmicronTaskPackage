...
namespace _GenericPackageStart.Core.CustomAttributes
{
    ...
    public class FindInSceneAttributeDrawer : PropertyDrawer
    {
            ...
            if (property.objectReferenceValue == null)
            {
                if (GUI.Button(buttonRect, "Find", greenButtonStyle))
                {
                    ...
                    if (script.gameObject != null)
                    {
                        ...
                        var component = GameObject.FindObjectsOfType(fieldType).FirstOrDefault(x => attribute.IsContainInName(((Component) x).gameObject.name));
