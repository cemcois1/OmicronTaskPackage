...
namespace PaintIn3D
{
    ...
    public class P3dModifyColorRandom : P3dModifier
    {
        ...
        [SerializeField] private BlendType blend;
        ...
        {
            gradient = UnityEditor.EditorGUILayout.GradientField(
                new GUIContent("Gradient",
                    "This is the gradient containing all the possible colors. A color will be randomly picked from this."),
                Gradient);
            blend = (BlendType) UnityEditor.EditorGUILayout.EnumPopup(
                new GUIContent("Blend", "The way the picked color value will be blended with the current one."), blend);
        }
#endif
    }
}